...
module.exports = {
  ...
  middleware: function(req, res, next) {
    const user = req.user;
    if (!user.dropbox.token) return next("User has not added dropbox");
    if (!dropbox) dropbox = require("../config/dropbox")(user.dropbox.token);
    return next();
  },
    ...
    return dropbox
      ...
      .then(result => {
        return res.status(200).send({
          ...
          total:
            Math.round((result.allocation.allocated * 100) / 1000000000) / 100
        });
      })
      .catch(err => next(err));
  },
  ...
  upload: function(req, res, next) {
    // if (!req.body.file) return next(new Error("No file provided"));
    // if (!req.body.path) return next(new Error("No path provided"));

    return dropbox
      .filesUpload({
        path: `/${req.files.file[0].originalname}`,
        contents: req.files.file[0].buffer
      })
